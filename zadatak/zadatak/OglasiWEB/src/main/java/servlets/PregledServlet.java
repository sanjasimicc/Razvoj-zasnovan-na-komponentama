...
/**
 * Servlet implementation class PregledServlet
 */
@WebServlet("/PregledServlet")
public class PregledServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		String pretrazi = request.getParameter("pretrazi");
		List<Ogla> oglasi = bean.sviOglasi(pretrazi);
		if(oglasi.isEmpty()) {
			request.getRequestDispatcher("error.jsp").forward(request, response);
		} else {
			request.setAttribute("bean2", bean);
			request.setAttribute("oglasi", oglasi);
			request.getRequestDispatcher("/PregledOglasa.jsp").forward(request, response);
		}
